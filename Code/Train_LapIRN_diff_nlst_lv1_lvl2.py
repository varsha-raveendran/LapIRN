...
model_name = "LDR_NLST_NCC_resize_"
wandb.login()
...
def train_lvl1():
    ...
    model = LapIRN_lvl1(2, 3, start_channel, is_train=True, imgshape=imgshape_2, range_flow=range_flow).to(device)
    ...
    model_dir = '../Model_2lvl_resize/Stage'
    ...
    #     model_path = "../Model_2lvl_resizes/LDR_LPBA_NCC_lap_share_preact_1_05_3000.pth"
    ...
    while step <= iteration_lvl1:
        ...
        for batch_idx, data in enumerate(training_generator):
            ...
            F_X_Y, X_Y, Y_2, F_xy, _= model(X.unsqueeze(dim=0), Y.unsqueeze(dim=0))
            # 3 level deep supervision NCC
            loss_multiNCC = loss_similarity(X_Y,Y_2)
...
#     model_path = sorted(glob.glob("/home/iml/varsha.raveendran/code/LapIRN/Model_2lvl_resizes/Stage/" + model_name + "stagelvl1_?????.pth"))[-1]
...
#     model = Miccai2020_LDR_laplacian_unit_add_lvl2(2, 3, start_channel, is_train=True, imgshape=imgshape,
...
#     optimizer = torch.optim.Adam(model.parameters(), lr=lr)
#     model_dir = '../Model_2lvl_resizes/Stage'
...
def train_lvl3():
    ...
    model_lvl1 = LapIRN_lvl1(2, 3, start_channel, is_train=True, imgshape=imgshape_2,
                                               range_flow=range_flow).to(device)
    ...
    model_path = sorted(glob.glob("../Model_2lvl_resize/Stage/" + model_name + "stagelvl1_?????.pth"))[-1]
    model_lvl1.load_state_dict(torch.load(model_path))
    print("Loading weight for model_lvl1...", model_path)
    ...
    load_model = False
    if load_model is True:
        model_path = "../Model_2lvl_resizes/LDR_OASIS_NCC_unit_add_reg_3_anti_1_stagelvl3_10000.pth"
        print("Loading weight: ", model_path)
        step = 10000
        model.load_state_dict(torch.load(model_path))
        temp_lossall = np.load("../Model_2lvl_resizes/lossLDR_OASIS_NCC_unit_add_reg_3_anti_1_stagelvl3_10000.npy")
    ...
    while step <= iteration_lvl3:
        ...
        for batch_idx, data in enumerate(training_generator):
            ...
            F_X_Y, X_Y, Y_4x, F_xy = model(X.unsqueeze(dim=0), Y.unsqueeze(dim=0))
            ...
            if (step % n_checkpoint == 0):
                ...
                with torch.no_grad():
                    ...
                    for batch_idx, data in enumerate(valid_generator):
                        ...
                        F_X_Y, *_ = model(X.unsqueeze(dim=0), Y.unsqueeze(dim=0))
                        X_Y = transform(X.unsqueeze(0), F_X_Y.permute(0, 2, 3, 4, 1), grid_unit).data.cpu().numpy()[0, 0, :, :, :]
                        ...
                        #TODO: check flow_x and flow_y indices to wandb
...
if not os.path.isdir("../Log_nlst_lvl2_resize"):
    os.mkdir("../Log_nlst_lvl2_resize")

log_dir = "../Log_nlst_lvl2_resize/" + model_name + ".txt"
