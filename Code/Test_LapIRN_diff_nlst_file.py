...
def test():
    ...
    with torch.no_grad():
        F_X_Y = model(moving_img.to(device), fixed_img.to(device))
        print(F_X_Y.shape)
        X_Y = transform(moving_img.to(device), F_X_Y.permute(0, 2, 3, 4, 1), grid).data.cpu().numpy()[0, 0, :, :, :] #grid sample
        ...
        save_flow(F_X_Y_cpu.numpy().transpose(3,0,1,2), savepath+'/flow_0047_0047.nii.gz')
